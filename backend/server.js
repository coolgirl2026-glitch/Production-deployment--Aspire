...
app.post("/api/auth/signup", async (req, res) => {
  ...
  try {
    ...
    const existing = await findLoginAccountByEmail(email.trim());
    if (existing) {
      return res.status(409).json({ error: "An account with this email already exists. Try signing in." });
    }

    const passwordHash = await hashPassword(password);
    const account = await createLoginAccount({ name: name.trim(), email: email.trim(), passwordHash });

    const token = signToken(account);
    return res.status(201).json({ token, user: { id: account.id, name: account.name, email: account.email } });
  } catch (err) {
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    const token = signToken(account);
    return res.status(200).json({ token, user: { id: account.id, name: account.name, email: account.email } });
  } catch (err) {
...
// expires (30 days) or they sign out.
